...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  // Search for an existing output entry with the same doubled value
  for (int i = 0; i < *result_count; i++) {
    if (output[i].doubled_value == input->doubled_value) {
      // Found a match: append this line number to the existing entry
  ...
  // No match found: create a new entry in the output array
